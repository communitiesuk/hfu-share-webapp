...
class SearchSponsorsTestCase(TestCase):
    ...
    def test_search_by_phone_number(self):
        results = perform_search(
            "000", Sponsor.objects.all(), DEDUP_SPONSORS_SEARCH_FIELDS
        )
    ...
    def test_search_by_email(self):
        results = perform_search(
            "sponsor1@example.com", Sponsor.objects.all(), DEDUP_SPONSORS_SEARCH_FIELDS
        )
    ...
    def test_search_by_full_name(self):
        results = perform_search(
            "John Doe", Sponsor.objects.all(), DEDUP_SPONSORS_SEARCH_FIELDS
        )

        self.assertTrue(self._check_object_in_results(self.sponsor1, results))
        self.assertFalse(self._check_object_in_results(self.sponsor2, results))

    def test_search_by_residential_postcode(self):
        results = perform_search(
            "OX1 1OX", Sponsor.objects.all(), DEDUP_SPONSORS_SEARCH_FIELDS
        )
    ...
    def test_search_no_match_should_return_no_results(self):
        results = perform_search(
            "Nonexistent", Sponsor.objects.all(), DEDUP_SPONSORS_SEARCH_FIELDS
        )
    ...
    def test_search_empty_query_should_return_all(self):
        # Empty query
        results = perform_search(
            "", Sponsor.objects.all(), DEDUP_SPONSORS_SEARCH_FIELDS
        )
    ...
    def test_search_accepts_short_params(self):
        results = perform_search(
            "r1", Sponsor.objects.all(), DEDUP_SPONSORS_SEARCH_FIELDS
        )
    ...
    def test_search_partial_by_partial_value(self):
        ...
        results = perform_search(
            "sponsor", Sponsor.objects.all(), DEDUP_SPONSORS_SEARCH_FIELDS
        )
    ...
    def test_search_by_multiple_keywords(self):
        # Tokenised search
        results = perform_search(
            "Jane,- John", Sponsor.objects.all(), DEDUP_SPONSORS_SEARCH_FIELDS
        )
