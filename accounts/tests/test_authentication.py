...
class LinkEntraIdentityTestCase(TestCase):
    ...
    def test_reattaches_a_user_whose_entra_object_id_has_changed(self):
        existing_user = UserFactory(email=EMAIL, username=EMAIL)

        user = self.authenticate()

        self.assertEqual(user.pk, existing_user.pk)
        existing_user.refresh_from_db()
        self.assertEqual(str(existing_user.entra_oid), OBJECT_ID)
        self.assertEqual(str(existing_user.entra_tid), TENANT_ID)
        self.assertEqual(User.objects.count(), 1)
